...
# Explanation : The positive numbers are [5, 2, 4, 7, 1, 8, 0] and the negative integers are [-5,-2,-8].
# According to the given conditions we will start from the positive integer 5 and then -5 and so on.
# After reaching -8 there are no negative elements left, so according to the given rule,
# we will add the remaining elements (in this case positive elements are remaining) as it in by maintaining the relative order.
# Input: arr[] = [9, 5, -2, -1, 5, 0, -5, -3, 2]
# Output: [9, -2, 5, -1, 5, -5, 0, -3, 2]
# Explanation: The positive numbers are [9, 5, 5, 0, 2] and the negative integers are [-2, -1, -5, -3]. Since, 
# we need to start with the positive integer first and then negative integer and so on (by maintaining the relative order as well),
#  hence we will take 9 from the positive set of elements and then -2 after that 5 and then -1 and so on.
...
def rearrange(arr):
    ...
    # reamining possitive append into array
    while p < len(pos):
        arr[index] = pos[p]
        index += 1
        p += 1
        
    # remainning negative append
    while g < len(neg):
        arr[index] = neg[g]
        index += 1
        g += 1
